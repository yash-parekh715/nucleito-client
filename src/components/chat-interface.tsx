...
//       <div className="w-full px-40"></div>
...
//                 className="mb-6 space-y-4 max-h-[400px] overflow-y-auto pr-0 custom-scrollbar"
//               >
...
export function ChatInterface() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Call the API endpoint
      const response = await fetch("http://192.168.0.171:3001/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ input: userMessage.content }),
      });
